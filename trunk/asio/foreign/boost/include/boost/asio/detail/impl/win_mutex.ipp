...
# endif
  ...
  return 0;
	...
	int win_mutex::do_init()
	{
		if (
			#ifndef NDEBUG
				::InitializeCriticalSectionEx(&crit_section_, static_cast<DWORD>(1u) << 14, 0)
			#else
				::InitializeCriticalSectionEx(&crit_section_, static_cast<DWORD>(1u) << 14, CRITICAL_SECTION_NO_DEBUG_INFO)
			#endif
		)
			return 0;
		else 
