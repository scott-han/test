...
class Multiplexer : public std::enable_shared_from_this<Multiplexer<Protocol>> {
 ...
 public:
  ...
  void Stop(boost::system::error_code &ec) {
    if (running_.load()) {
      ...
      {
        ::std::lock_guard<synapse::misc::mutex> lock_socket(socket_mutex_);
  ...
  SocketSessionPtr CreateSocketSession(boost::system::error_code &ec,
    ...
    ::std::lock_guard<synapse::misc::recursive_mutex> lock_sockets_map(sessions_mutex_);
    SocketId id(0);
    if (user_socket_id == 0) {
    ...
    if (id == 0) {
      ec.assign(udt::common::error::address_not_available,
  ...
  void RemoveSocketSession(const NextEndpoint &next_remote_endpoint,
    ...
    ::std::lock_guard<synapse::misc::recursive_mutex> lock_sockets_map(sessions_mutex_);
    ::std::lock_guard<synapse::misc::recursive_mutex> lock_flows(flows_mutex_);
  ...
  void SetAcceptor(boost::system::error_code &ec,
    ...
    ::std::lock_guard<synapse::misc::mutex> lock_acceptor(acceptor_mutex_);
    if (p_acceptor_) {
      ec.assign(udt::common::error::address_in_use,
                udt::common::error::get_error_category());
      return;
    }
  ...
  void RemoveAcceptor() {
    ::std::lock_guard<synapse::misc::recursive_mutex> lock_sessions(sessions_mutex_);
    ::std::lock_guard<synapse::misc::mutex> lock_acceptor(acceptor_mutex_);
    ...
    if (p_datagram->is_acked()) {
      this->get_io_service().post(
          boost::asio::detail::binder2<decltype(handler),
              ...
              handler,
              boost::system::error_code(udt::common::error::identifier_removed,
                                        udt::common::error::get_error_category()),
              0));
      return;
    }
  ...
  void HandlePacket(GenericDatagramPtr p_generic_packet,
    ...
    if (header.IsControlPacket()) {
      ...
      if (control_datagram.header().IsType(
        ...
        {
          ::std::lock_guard<synapse::misc::mutex> lock(acceptor_mutex_);
 ...
 private:
  ...
  synapse::misc::mutex socket_mutex_;
  NextSocket socket_;
  std::unique_ptr<boost::asio::io_service::work> p_worker_;
  std::atomic<bool> running_;
  synapse::misc::recursive_mutex flows_mutex_;
  FlowsMap flows_;
  synapse::misc::recursive_mutex sessions_mutex_;
  RemoteEndpointFlowMap remote_endpoint_flow_sessions_;
  synapse::misc::mutex acceptor_mutex_;
