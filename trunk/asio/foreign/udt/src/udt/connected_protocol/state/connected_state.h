...
class ConnectedState : public BaseState<Protocol>,
 ...
 public:
  ...
  virtual void Init() {
    ...
    nack_timer_.async_wait(boost::bind(&ConnectedState::NAckTimerHandler, this->shared_from_this(), _1));
  }
  ...
  virtual void OnControlDgr(ControlDatagram* p_control_dgr) {
    ...
    switch (p_control_dgr->header().flags()) {
      ...
      case ControlDatagram::Header::HACK: {
        ResetExp(false);
				::std::cerr << "HACK!" << ::std::endl;
				congestion_control_.OnHack(ntohl(p_control_dgr->header().additional_info()));
        break;
      }
 ...
 private:
  ...
  void AckTimerHandler(const boost::system::error_code& ,
    ...
    p_session->AsyncSendControlPacket(
        *p_ack_datagram, AckDatagram::Header::ACK, ack_seq_num,
		...
		if (!light_ack)
			::std::cerr << "sent full ack" << ::std::endl;
  }
  ...
  void ExpTimerHandler(const boost::system::error_code& ec) {
    ...
    if (ec) {
      ...
      return;
		...
		::std::cerr << " sender blahs " << sender_.blah << ::std::endl;
		::std::cerr << " sender windows full " << sender_.last_window_full << ::std::endl;
		::std::cerr << " congestion w " << sender_.congestion_window << ::std::endl;
		::std::cerr << " session w " << sender_.session_window << ::std::endl;
		::std::cerr << " flow blahs " << p_session->p_flow_->blah << ::std::endl;
		::std::cerr << " flow timeout " << p_session->p_flow_->timeout___ << ::std::endl;
    ...
    if (!sender_.HasLossPackets()) {
      sender_.UpdateLossListFromNackPackets();
		...
		::std::cerr << " sender has packet to send " << sender_.HasPacketToSend() << ::std::endl;
		::std::cerr << " flow is pulling flag " << p_session->p_flow_->pulling_ << ::std::endl;
    ...
    if (receiver_.HasTimeout()) {
      ...
      Close();
		::std::cerr << " CLOSE RECEIVER TIMEOUT " << ::std::endl;
      return;
    }
 ...
 private:
  void OnAck(const AckDatagram& ack_dgr) {
    ...
    if (payload.IsLightAck()) {
			auto const dist(packet_seq_gen.distance(receiver_.largest_acknowledged_seq_number(), packet_ack_number));
      if (dist >= 0) {
        // available buffer size in packets...
        // update remote window flow
        //p_session->set_window_flow_size(p_session->window_flow_size() - dist);
    ...
    if (packet_seq_gen.distance(receiver_.largest_acknowledged_seq_number(), packet_ack_number) >= 0) {
      receiver_.set_largest_acknowledged_seq_number(packet_ack_number);
      // available buffer size in packets
  ...
  void OnAckOfAck(const AckOfAckDatagram& ack_of_ack_dgr) {
    ...
    if (acked) {
      if (packet_seq_gen.distance(receiver_.largest_ack_number_acknowledged(), packet_seq_num) > 0) {
