...
class AcceptorSession {
 ...
 public:
  ...
  void Close() {
    ::std::lock_guard<synapse::misc::recursive_mutex> lock_sessions_accept_ops(mutex_);
    if (!p_multiplexer_) {
      return;
    }
    ...
    boost::system::error_code ec(udt::common::error::interrupted,
                                 udt::common::error::get_error_category());
    CleanAcceptOps(ec);
  ...
  void PushAcceptOp(AcceptOp* p_accept_op) {
    {
      ::std::lock_guard<synapse::misc::recursive_mutex> lock_accept_ops(mutex_);
  ...
  void PushConnectionDgr(ConnectionDatagramPtr p_connection_dgr,
    ...
    if (!p_socket_session) {
      ...
      {
        ::std::lock_guard<synapse::misc::recursive_mutex> lock_sessions(mutex_);
 ...
 private:
  void Accept() {
    ...
    if (!connected_sessions_.empty() && !accept_ops_.empty()) {
      boost::system::error_code ec(udt::common::error::success,
                                   udt::common::error::get_error_category());
