...
class basic_DataHeader {
 private:
  ...
  struct Content {
    ...
    uint32_t packet_sequence_number() const { return data[0]; }
    ...
    uint32_t packet_info() const { return data[1]; }
    /// 32 bits: timestamp when the packet is sent
    /// Relative value starting from the time when the connection is set up
    uint32_t timestamp() const { return data[2]; }
    /// 32 bits: id of the socket for UDT multiplexer
    uint32_t destination_socket() const { return data[3]; }

    void packet_sequence_number(uint32_t x) { data[0] = x; }
    void packet_info(uint32_t x) { data[1] = x; }
    void timestamp(uint32_t x) { data[2] = x; }
    void destination_socket(uint32_t x) { data[3] = x; }
  };
 ...
 public:
  enum position : uint32_t {
    ...
    LAST = 0x40000000,
    FIRST = 0x80000000,
    ONLY_ONE_PACKET = 0xC0000000 // 0xB0000000
  };
  ...
  basic_DataHeader &operator=(basic_DataHeader const & other) {
		static_assert(sizeof(content_.data) == sizeof(other.content_.data), "content_.data");
    ::memcpy(content_.data, other.content_.data, sizeof(content_.data));
    return *this;
  }
  ...
  void GetMutableBuffers(MutableBuffers *p_buffers) {
    p_buffers->push_back(boost::asio::buffer(content_.data, sizeof(content_.data)));
  }
  ...
  void set_packet_sequence_number(uint32_t packet_sequence_number) {
    content_.packet_sequence_number(htonl((ntohl(content_.packet_sequence_number()) & 0x80000000) | packet_sequence_number));
  }
  ...
  void set_message_position(position pos) {
    content_.packet_info(htonl((ntohl(content_.packet_info()) & 0x3FFFFFFF) | pos));
  }
  ...
  void set_in_order(order in_order) {
    content_.packet_info(htonl((ntohl(content_.packet_info()) & 0xDFFFFFFF) | in_order));
  }
  ...
  void set_message_number(uint32_t message_number) {
    content_.packet_info(htonl((ntohl(content_.packet_info()) & 0xE0000000) | message_number));
  }
...
class basic_ControlHeader {
 ...
 private:
  struct Content {
    ...
    uint32_t flags_() const { return data[0]; }
    /// 3-32 bits: function of the type
    uint32_t additional_info_() const { return data[1]; }
    /// 32 bits timestamp when the packet is sent
    /// Relative value starting from the time when the connection is set up
    uint32_t timestamp_() const { return data[2]; }
    /// 32 bits: id of the socket for UDT multiplexer
    uint32_t destination_socket_() const { return data[3]; }
    void flags_(uint32_t x) { data[0] = x; }
    void additional_info_(uint32_t x) { data[1] = x; }
    void timestamp_(uint32_t x) { data[2] = x; }
    void destination_socket_(uint32_t x) { data[3] = x; }
  };
 ...
 public:
  ...
  void set_additional_info(uint32_t additional_info) {
    content_.additional_info_(htonl((ntohl(content_.additional_info_()) & 0xE0000000) | additional_info));
  }
