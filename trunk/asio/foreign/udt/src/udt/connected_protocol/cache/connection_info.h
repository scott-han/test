...
class ConnectionInfo {
 ...
 public:
	unsigned constexpr static min_window_size{8192};
  ConnectionInfo(long long syn_interval = 10000)
      : syn_interval_(syn_interval),
        ...
        //sending_period_(0.0),
        //window_flow_size_(min_window_size),
        ack_period_(syn_interval),
        nack_period_(syn_interval),
        exp_period_(500000),
  ...
  boost::chrono::microseconds rtt() const {
    return boost::chrono::microseconds(rtt_.load());
  }
  ...
  boost::chrono::microseconds rtt_var() const {
    return boost::chrono::microseconds(rtt_var_.load());
  }

  boost::chrono::microseconds ack_period() const {
    return boost::chrono::microseconds(ack_period_.load());
  }

  void UpdateAckPeriod() {
    //ack_period_ = (3 * rtt_.load() + rtt_var_.load() + syn_interval_.load());
    ack_period_ = 3 * rtt_.load();
  }

  boost::chrono::microseconds nack_period() const {
    return boost::chrono::microseconds(nack_period_.load());
  }

  void UpdateNAckPeriod() {
    //nack_period_ = (3 * rtt_.load() + rtt_var_.load() + syn_interval_.load());
    nack_period_ = (3 * rtt_.load() + rtt_var_.load() + syn_interval_.load());
  }
  ...
  boost::chrono::microseconds exp_period() const {
    return boost::chrono::microseconds(exp_period_.load());
  }
