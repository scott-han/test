...
./amqp_0_9_1/clients/Cpp/${BASENAME}.exe --Action Corrupt --Topics_Path Synapse_database.tmp > Client_Corrupting_Log.txt 2>&1 || Bail "Corruption error"

./database/basic_recover.exe --src Synapse_database.tmp --dst Synapse_database.tmp  --inhibit_delta_poisoning false > Basic_Recover_Log.txt 2>&1 || Bail "Basic_recover error"
